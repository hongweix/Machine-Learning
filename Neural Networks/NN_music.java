import java.io.*;
import java.util.*;
//refered idea from "http://fantasticinblur.iteye.com/blog/1465497" 
//for construct the matrix and do calculation and adjust weight
public class NN_music{
	private double[] inputLayer = new double[4];
	private double[] hiddenLayer = new double[6];
	private double[] hiddenDelta = new double[6];
	private double[][] inputhiddenWeight = new double[4][6];
	private double[][] inputhiddenPreWeight = new double[4][6];
	...
	public static void main(String[] args) throws IOException{
		...
		while((newline = br.readLine())!=null){
			...
			for(String each: variables){
				...
				else if(each.equals("no")){
					record[i] = 0.0;
				}
				else{
					if(i == 0){
						record[i] = (Double.parseDouble(each) - 1900)/100;
					}
					else{
						record[i] = Double.parseDouble(each)/7;
					}
				}
				i += 1;
			}
		...
		while(nnm.sumError > 1){
			//set sumError to be 0 each loop
			nnm.sumError = 0;
			for(int i = 0;i < data.size();i++){
				double[] record = data.get(i);
				nnm.inputLayer = new double[]{record[0],record[1],record[2],record[3]};
				nnm.targetValue = record[4];
				nnm.feedforward();
				nnm.calculateError();
				nnm.adjustWeight();
			}
		...
		while((newline = br.readLine())!=null){
			...
			nnm.feedforward();
			System.out.println(nnm.outputLayer > 0.5 ? "yes":"no");
		}
		br.close();
	}
}