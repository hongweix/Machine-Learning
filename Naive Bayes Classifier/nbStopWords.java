...
public class nbStopWords{
	public static void main(String[] args) throws IOException {
		...
		//sort the words map
		LinkedList<Map.Entry<String, Integer>> wordsList = new LinkedList<>(conWords.entrySet());
		wordsList.addAll(libWords.entrySet());
		...
		for(int i = 0;i < N;i++){
			String word = wordsList.remove().getKey();
		...
		for(String stopWord: stopWords){
			...
			if(conWords.containsKey(stopWord)){
				conWordsCount -= conWords.remove(stopWord);
			}
			if(libWords.containsKey(stopWord)){
				libWordsCount -= libWords.remove(stopWord);
			}
		}
		...
		for(Map.Entry<String, Integer> each: conWords.entrySet()){
			conWordsProb.put(each.getKey(), (double)(each.getValue() + 1) / (conWordsCount + vocabulary.size()));
		}
		//words with probability in liberal blog
		for(Map.Entry<String, Integer> each: libWords.entrySet()){
			libWordsProb.put(each.getKey(), (double)(each.getValue() + 1) / (libWordsCount + vocabulary.size()));
		}
		...
		while((eachfile = br.readLine()) != null){
			...
			while((newline = br1.readLine()) != null){
				...
				if(conWordsProb.containsKey(newline) && !libWordsProb.containsKey(newline)){
					...
					libProduct += Math.log((double)1/(libWordsCount + vocabulary.size()));
				}
				else if(!conWordsProb.containsKey(newline) && libWordsProb.containsKey(newline)){
					conProduct += Math.log((double)1/(conWordsCount + vocabulary.size()));
